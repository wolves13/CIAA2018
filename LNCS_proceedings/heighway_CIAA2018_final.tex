...
and by JSPS and NRF under the Japan-Korea Basic Scientific Cooperation Program No.~YB29004.
}}
...
An RNA polymerase attaches to a template DNA sequence (double-spiral), scans it through, and synthesizes its RNA copy. 
...
Finite portions of the Sierpinski triangle fractal was algorithmically self-assembled even \textit{in vitro} from coalescence of DNA tiles that compute XOR \cite{RothemundPapadakisWinfree2004}. 
...
\centering
\scalebox{0.0375}{\begin{tikzpicture}
  \draw[ultra thick, rotate=270,l-system={rule set={X->X-YF,Y->FX+Y},
...
\draw[ultra thick] (\x, 0) node[mol] {} node[above] {\Large $a$}
-- ++(300:1) node[mol] {} 
...
;
\draw[ultra thick, dotted] (\x, 0) -- ++(0:1);
}

\draw[dashed, -latex] (1, 0) -- ++(120:1) node[mol] {} node[above] {\Large $a$} -- ++(180:1) node[mol]{} -- ++(240:1) node[mol] {} node[above] {\Large $b'$};
\draw[dashed, -latex] (1, 0) -- ++(60:1) node[mol] {} node[above] {\Large $a$} -- ++(0:1) node[mol] {} -- ++(300:1) node[mol] {} node[right] {\Large $b'$};
\draw[ultra thick, dashed, -latex] (1, 0) -- ++(0:1) node[mol] {} node[above] {\huge $a$} -- ++(300:1) node[mol] {} -- ++(240:1) node[mol] {} node[below] {\huge $b'$};

\draw[dotted] (0,0)++(300:2) -- ++(0:1);
...
\draw[ultra thick, -latex] (5, 0) -- ++(0:1) node[mol] {} node[above] {\Large $a$};

\draw[ultra thick, dashed, -latex] (6, 0) -- ++(300:1) node[mol] {} -- ++(240:1) node[mol] {} node[below] {\Large $b'$} -- ++(0:1) node[mol] {} node[below] {\Large $b$};
\draw[ultra thick, dashed, -latex] (5,0)++(300:2)++(240:0.3) -- ++(240:0.7) node[mol] {} node[below] {\Large $b$};
\draw[ultra thick, dashed, -latex] (5,0)++(300:2.3) -- ++(300:0.7) node[mol] {} node[below] {\Large $b$};
\draw[dotted] (4,0)++(300:2) -- ++(0:1);


\draw[ultra thick, -latex] (9, 0) -- ++(0:1) node[mol] {} node[above] {\Large $a$}
-- ++(300:1) node[mol] {}
;
\draw[ultra thick, dashed, -latex] (10,0)++(300:1) -- ++(240:1) node[mol] {} node[below] {\Large $b'$}
-- ++(240:1) node[mol] {} node[below] {\Large $b$}
-- ++(180:1) node[mol] {}
;
\draw[ultra thick, dashed, -latex] (9,0)++(300:2) -- ++(0:1) node[mol] {} node[below] {\Large $b$} -- ++(60:1) node[mol] {};
\draw[dashed, -latex] (10,0)++(300:1) -- ++(60:1) node[mol] {} node[right] {\Large $b'$}
-- ++(120:1) node[mol] {} node[above] {\Large $b$}
-- ++(180:1) node[mol] {}
;
\draw[dotted] (8,0)++(300:2) -- ++(0:1);

\draw[ultra thick, -latex] (14, 0) -- ++(0:1) node[mol] {} node[above] {\Large $a$}
-- ++(300:1) node[mol] {}
-- ++(240:1) node[mol] {} node[below] {\Large $b'$}
;
\draw[ultra thick, dotted] (13,0)++(300:2) -- ++(0:1);
\draw[ultra thick, dashed, -latex] (14,0)++(300:2) -- ++(0:1) node[mol] {} node[below] {\Large $b$}
-- ++(60:1) node[mol] {}
-- ++(120:1) node[mol] {} node[above] {\Large $a'$}
;
\draw[ultra thick, dotted] (15,0) -- ++(0:1);
...
Its seed, bold in Fig.~\ref{fig:glider} (left), can be elongated by the first three beads $w[1..3] = a \bullet b'$ in various ways, only three of which are shown as a dashed arrow. 
Only $w[3] = b'$ can form a new h-interaction (dotted line), with the $b$ in the seed (according to $\mathcal{H}$, the first $a$ is also capable of binding, with $a'$, but the sole $a'$ around is just ``too close''). 
For the $b{-}b'$ binding, $w[1..3]$ must be folded as the bold dashed arrow. % in Fig.~\ref{fig:glider} (left). 
...
Independently of $j_1, j_2$, both delay and arity are set to 3 and 567 bead types\footnote{Some of the bead types might be saved but not easily due to the NP-hardness of minimizing the number of bead types without changing the behavior \cite{HanKim2017}.} \texttt{1}, \texttt{2}, \dots, \texttt{567} with a fixed rule set $\mathcal{H}$ are employed. 
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.92\linewidth]{Figs/dragon_vol5.png}
\caption{
