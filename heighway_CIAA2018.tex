...
\draw(6.5, 0)++(300:1.5) node {Turner $T$};
...
\draw(-0.75, 0)++(300:3.75) node {Turner $T$};
...
\paragraph{Modularization} is a factorization of transcript into functional units. 
...
\item $T$ is a \textit{turner module}; it makes a turn according to the interpretation. 
\end{itemize}
...
Each of the modules consists of small submodules, say, with several dozens of beads. 
...
\paragraph{Seed} encodes the initial count $i = j_1$ in its binary representation $b_n b_{n-1} \cdots b_1$ as: 
...
$D_v$ employs the six submodules: {\tt Dzig1}, \texttt{Dzag1}, \texttt{Dzig2}, \texttt{Dzag2}, \texttt{PFS}, and $\mathtt{AO}_v$ (resp.~$\mathtt{AO}_h$), which are interleaved by spacers, as well as those that guide the transcript into two zigzags and one more zig. 
...
\texttt{Dzig1} knows which phase it is in by the relative position where it starts folding to the input above (far in Phase1, close in Phase 2). 
...
In the first zag, $n$ copies of \texttt{Dzag1} reformat and propagate 0's, 1's, and the first 0 using three bricks in Fig.~\ref{fig:Dzag1}.
...
The next \texttt{Dzig2}, if any, starts folding at the top so that it takes the special brick Dzig2-0f0 if the first 0 is followed by 0 or Dzig2-1f0 if it is follwed by 1. 
...
\paragraph{Turner module} $T$ consists of 3 copies of the pair of two functional units called \textit{bit-bifurcator} and \textit{steering arm}. 
See Fig.~\ref{fig:turner}. 
The bit-bifurcator bifurcates the current count $i = a_n a_{n-1} \cdots a_1$ as well as the A/O signal into the left and right while folding into zigzags. 
...
The submodule \texttt{body-rgy} is implemented by reusing the first half of \texttt{Dzag2} (Fig.~\ref{fig:Dzag2}). 
...
The \texttt{body-lgy} and \texttt{turn-lgp} are the zig-counterparts of them (Fig.~\ref{fig:body-lgy} and \ref{fig:turn-lgp}). 
...
The first and second halves are diverted to implement the \texttt{body-gx1} (T-E) and \texttt{body-rgy} (T-G) submodules of the turner module, respectively.}
\label{fig:Dzag2}
\end{figure}
...
Inside the main text, we have already shown the bricks of the \texttt{turn-rgp} (T-I) and \texttt{change-route} (T-K) submodules of the turner module (Figs.~\ref{fig:turn-rgp} and \ref{fig:change_route}). 
...
The formatter \text{CF} and half-adder \texttt{HA} of the counter module are diverted respectively as the \texttt{body-rpx2} (T-B) and \texttt{body-lpx2} (T-D) submodules. 
...
\caption{The brick automaton for the first zag of Turner module.}
...
\caption{The brick automaton for all zags that involve \texttt{body-rpx1} (T-A) below a zig that involves \texttt{body-gx1} (T-E) of Turner module.}
...
\caption{The brick automaton for all zags that involve \texttt{body-rpx2} (T-B) of Turner module.}
\label{fig:BA_T-B}
\end{figure}
...
\caption{The brick automaton for all zigs that involve \texttt{body-lpx1} (T-C) of Turner module.}
\label{fig:BA_T-C}
\end{figure}
...
\caption{The brick automaton for all zigs that involve \texttt{body-lpx2} (T-D) of Turner module.}
\label{fig:BA_T-D}
\end{figure}
...
\caption{The brick automaton for all zags that involve \texttt{body-gx1} (T-E) of Turner module, which also involve one \texttt{body-rgy} (T-G) and $n-1$ \texttt{body-gx2} (T-F).}
...
\caption{The brick automaton for all zigs that involve \texttt{body-gx1} (T-E) of Turner module, which also involve one \texttt{body-lgy} (T-H).}
